...
class metafield(property, t.Generic[_TF]):
	...
	def __load__(self, obj) -> t.Union[_TF, t.Any]:
		...
		if self.fload is None:
			...
			if self.inherit and base is not None and self.__name__ in base:
...
class BaseMetadata(t.Generic[T], metaclass=MetadataType):
	...
	def __init__(self, target = None, name=None, raw=None, base=None):
		...
		# self.__name__ = name
		self.__base__ = base
		self.__raw__ = dict() if raw is None\
					else self._raw_from_type(raw) if isinstance(raw, type) else raw

		name and target and setattr(target, name, self) 
	...
	def _base_from_target(cls, target, attr):
		...
		maps = (b.__dict__ for b in maps if isinstance(b, BaseMetadata))
		return ChainMap({}, *maps)

	@classmethod
	def _raw_from_type(cls, t):
		return {k: getattr(t, k) for k in dir(t) if k[:2] != '__'}
	...
	def __repr__(self) -> str:
		fields = ", ".join(f'{f}={getattr(self, f)}' for f in self.__fields__)
		return f'{self.__class__.__name__}({self.target.__name__}, {fields})'

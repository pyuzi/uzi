...
from ...fields import SiteForeignKey
from ...utils import get_model_by_db_table, get_site_column
...
class DatabaseSchemaEditor(PostgresqlDatabaseSchemaEditor):
    ...
    def _alter_field(self, model, old_field, new_field, old_type, new_type,
        ...
        if isinstance(new_field, SiteForeignKey) and new_field.db_constraint:
    ...
    def _create_fk_sql(self, model, field, suffix):
        if isinstance(field, SiteForeignKey):
            try:
            ...
            from_columns = field.column, get_site_column(from_model)
            to_columns = field.target_field.column, get_site_column(to_model)
            suffix = suffix % {

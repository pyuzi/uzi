...
from .utils import (get_current_site, get_site_column,
                    get_site_filters, is_distributed_model)


def add_site_filters_on_query(obj):
    current_site = get_current_site()

    if current_site:
        try:
            filters = get_site_filters(obj.model)
            obj.add_q(Q(
                **filters
            ))
        except ValueError:
...
def wrap_get_compiler(base_get_compiler):
    def get_compiler(obj, *args, **kwargs):
        add_site_filters_on_query(obj)
        return base_get_compiler(obj, *args, **kwargs)

    get_compiler._sign = "get_compiler djx-multisite"
    return get_compiler
...
def wrap_update_batch(base_update_batch):
    ...
    update_batch._sign = "update_batch djx-multisite"
    return update_batch

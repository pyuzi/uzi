...
class IocContainer:
    ...
    providers: fallback_default_dict[str, PriorityStack[Injectable, p.Provider]]
    dep_class: type[p.Provider]
    ...
    def bootstrap(self):
        ...
        print(f'Bootstraping: {self.__class__.__name__!r} --> {id(self)}')
    ...
    def register(self, 
            ...
            use: t.Union[p.Provider, T_UsingAny], /,
    ...
    def create_provider(self, provider: t.Union[p.Provider, T_UsingAny], **kwds: dict) -> p.Provider:
            ...
            for s, seq in self._iter_abstracts(tags):
                ...
                for tag in seq:
                    if not (tag in skip or skip.add(tag)):
                        ...
                        flush(tag, at)
                        original = bag.setdefault(tag, provider)
                        if original is provider:
                            logger.warning(f'++++> {tag!r} {at=!r} --> {provider!r}')
                            continue
                        logger.warning(f'==== {tag!r} {at=!r} --> {provider!r}')
                        raise DuplicateProviderError(f'{tag!r} {at=!r}, {provider=!r}, original={bag[tag]!r} {id(original)} <--> {id(provider)}')
                        # flush(tag, at)

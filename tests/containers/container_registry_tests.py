...
def test_basic(new: _T_FnNew, MockContainer: type[Container], MockGroup: type[Group]):
    ...
    con, con0 = (
        MockContainer(),
        MockContainer(_is_anonymous=True),
    )
...
def test_all(new: _T_FnNew, MockContainer: type[Container], MockGroup: type[Group]):
    ...
    print(
        f"  - byname_n_mods={test!r} {[*mods]} -->",
        [v.qualname for v in byname_n_mods],
        sep=sep,
    )
    print(
        f"  - bypattern={f'{test}:**'!r} -->", [v.qualname for v in bypattern], sep=sep
    )
    print(
        f"  - bypattern2={f'{test}:++'!r} -->",
        [v.qualname for v in bypattern2],
        sep=sep,
    )
    ...
    for test in mods:
        ...
        print(f"  - byname={test!r} -->", [v.qualname for v in byname], sep=sep)
        print(
            f"  - bypattern={f'{test}:**'!r} -->",
            [v.qualname for v in bypattern],
            sep=sep,
        )
        print(
            f"  - bypattern2={f'{test}:++'!r} -->",
        ...
        print(
            f"  - byregex=r{f'^{test}:.*'!r} -->",
            [v.qualname for v in byregex],
            sep=sep,
        )
    ...
    names = "g0", "c0"
    assert [*sub.all(*(f"**:{n}" for n in names))] == sorted(
        (v for v in values if v.name in names), key=lambda v: names.index(v.name)
    )

...
def test_find_provider(
    new: _T_FnNew, MockContainer: type[Container], MockProvider: type[Provider]
):
...
def test_overridden_final_find_provider(
    new: _T_FnNew, MockContainer: type[Container], MockProvider: type[Provider]
):
...
def test_getitem(
    ...
    print(
        "", sub_a[_Tb], sub_a[_Ta], sub_b[_Tb], sub_b[_Ta], sub_b[_Tx], sep="\n  ---> "
    )
...
def _test_find_local_existing(
    new: _T_FnNew, mock_graph: Graph, mock_provider: Provider
):
    mock_graph.__contains__.return_value = False
    sub = new(parent=mock_graph)

...
from uzi.graph.core import ResolutionStack
    ...
    with sub.push(mock_provider, _T):
        ...
        assert all(
            ...
            for x in (
                ...
                _T,
                mock_provider,
                (prov_container, _T, mock_provider),
                mock_container,
            )
        )
        assert all(
...
async def test_multiple_contexts(
    new: _T_FnNew, mock_container, MockProvider: type[Provider], MockContainer
):
    sub = new()

    cont1, cont2 = MockContainer(), MockContainer()
    prov1, prov2 = (
        MockProvider(container=cont1),
        MockProvider(container=cont2),
    )
    pushes = 0

...
def test_pro_entries(new_predicate: _T_New, cls: type[Predicate], MockDepSrc: type[DepSrc], MockScope: type[Scope], MockContainer):
    sub = new_predicate()
    n = 5
    containers = tuple(MockContainer() for _ in range(n))
    if sub is cls.only_self:
        src = MockDepSrc()
        assert sub.pro_entries(containers, src.scope, src) == containers
        assert sub.pro_entries(containers, MockScope(), src) == ()
    elif sub is cls.skip_self:
        src = MockDepSrc()
        assert sub.pro_entries(containers, src.scope, src) == ()
